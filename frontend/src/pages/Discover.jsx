    ...
    try {
      ...
      if (err.response) {
        if (err.response.status === 400) {
          setError(err.response.data.error || 'Invalid request. You may have already liked this profile.');
        } else if (err.response.status === 404) {
          setError('Profile not found. It may have been removed.');
      ...
      // Move to next profile despite the error
      goToNextProfile();
    }
  };
