...
const CountryPaymentMethodsManager = () => {
  ...
  const fetchConfiguredMethodsForCountry = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/api/admin/payment-configurations/country/${selectedCountryId}`);
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
        if (editingConfig.country_id && editingConfig.payment_method_id) { // Indicates editing existing config
            ...
            await api.put(`/api/admin/payment-configurations/country/${selectedCountryId}/method/${editingConfig.payment_method_id}`, payload);
        } else { // Adding new config
            await api.post(`/api/admin/payment-configurations/country/${selectedCountryId}`, payload);
        }
  ...
  const handleDeleteConfig = async (methodToDel) => {
    ...
    if (window.confirm(`Are you sure you want to remove ${methodToDel.payment_method_name} for ${selectedCountryName}?`)) {
        try {
            await api.delete(`/api/admin/payment-configurations/country/${selectedCountryId}/method/${methodToDel.payment_method_id}`);
            await fetchConfiguredMethodsForCountry();
        } catch (err) {
